...
class TelegramDriver extends HttpDriver {
  ...
  public function buildServicePayload($message, $matchingMessage, $additionalParameters = []) {
    ...
    if ($message instanceof Question) {
      ...
      // Custom add - for "editMessage" / "editMessageReplyMarkup"
      ...
      if (!empty($message->getReplyMarkup())) {
        // Если использовать editMessageReplyMarkup, то меняться будут только кнопки
    ...
    //   $parameters['reply_markup'] = $message->getReplyMarkup();
    // } elseif($message instanceof \App\Services\TelegramQuestionEditMessage) {
    //   // SERG custom add - for "editMessage"
    //   $parameters['message_id'] = $message->getChatMessageId();
    //   $parameters['text'] = $message->getText();
    //   if(!empty($message->getReplyMarkup())){
    //     $parameters['reply_markup'] = $message->getReplyMarkup();
    //   }

    //   if(empty($parameters['text']) && !empty($parameters['reply_markup'])){
    //     $this->endpoint = 'editMessageReplyMarkup';
    //   }else{
    //     $this->endpoint = 'editMessageText';
    //   }
  ...
  public function sendPayload($payload) {
    ...
    //info('TELEGRAM p', $payload);
    info('TELEGRAM '. $this->endpoint, $payload);
  ...
  public function sendRequest($endpoint, array $parameters, IncomingMessage $matchingMessage) {
    ...
    info('TG sendRequest', $parameters);
